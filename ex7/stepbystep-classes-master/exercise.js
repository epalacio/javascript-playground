...
class Basket {
  ...
  add(product, quantity) {
    ...
    // if product is already in the basket, add the quantity to the current quantity;
    const productQuantity = this.productQuantities[product.id];
    if(productQuantity) {
      productQuantity.quantity = productQuantity.quantity + quantity;
    } else {
      // Otherwise, add the product and quantity to productQuantities;
  ...
  remove(product, quantity) {
    // if the product is not in the basket or quantity <= 0, return;
    const productQuantity = this.productQuantities[product.id];
    if(!productQuantity || quantity <= 0) {
      return;
    }

    //if quantity is undefined || >= current quantity, remove item;
    if(typeof quantity === 'undefined' || quantity >= productQuantity.quantity) {
      delete this.productQuantities[product.id];
    } else {
      //else, subtract the quantity from the current quantity;
      productQuantity.quantity = productQuantity.quantity - quantity;
    } 
  }

  getTotal() {
    return Object.keys(this.productQuantities).reduce(
      (runningTotal, productId) => {
        const productQuantity = this.productQuantities[productId];
        return runningTotal + (productQuantity.product.price * productQuantity.quantity);
      }, 0
    );
  }
  ...
  getOrderLine(product) {
    ...
    return {
      ...
      subTotal: productQuantity.quantity * productQuantity.product.price,
    }
  }
}