...
function withTax(prices) {
	const newArr = prices.map(price => price >= 100 ? parseFloat((price * 1.2).toFixed(2)) : price);
	return newArr;
}
...
function totalPerRegion(perRegionPerQuarter) {
	const finals = perRegionPerQuarter.map(region => region.reduce((totals, city) => {
		return totals+= city;
	}, 0));
	return finals;
};